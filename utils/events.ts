...
export const getEventData = async (contract: Contract, eventName: string, filter: Array<string | null> | [],  startBlock?: number, endBlock?: number) => {

    if (!startBlock || !endBlock) return contract.queryFilter(contract.filters[eventName](...filter));
    ...
    for (let i = from; i < to; i += 5000) {
        ...
        const events = await contract.queryFilter(contract.filters[eventName](...filter), _startBlock, _endBlock);
        allEvents = [...allEvents, ...events]
    }
    return allEvents
}

export const getLiquidationCreatedEventData = async (contract: Contract, filter: Array<string | null> = []) => {
...
export const getLiquidationDisputedEventData = async (contract: Contract, filter: Array<string | null> = []) => {
...
export const getDisputeSettledEventData = async (contract: Contract, filter: Array<string | null> = []) => {
    const start = contractToBlockCreated[contract.address.toLowerCase()];
    const end = await contract.provider.getBlockNumber();
...
export const getLiquidationWithdrawnEventData = async (contract: Contract, filter: Array<string | null> = []) => {
    const start = contractToBlockCreated[contract.address.toLowerCase()];
    const end = await contract.provider.getBlockNumber();
